...
import random
import numpy as np
class PickleSafeSampler(BaseSampler):
    ...
    def __init__(self, graph_store: GraphStore, num_neighbors: List[int], ):
        self.instance_id = PickleSafeSampler._instance_counter
        PickleSafeSampler._instance_counter += 1
    ...
    def sample_from_nodes(self, ns_input: NodeSamplerInput) -> SamplerOutput:
        ...
        if worker_info is not None:
            ...
            if not hasattr(self, "_worker_initialized"):
                self.graph_store.reinitialize_driver()
